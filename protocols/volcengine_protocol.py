...
def full_client_request(payload: bytes) -> bytes:
    """Create full client request message"""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.NoSeq)
    msg.payload = payload
    logger.info(f"Creating: {msg}")
    return msg.marshal()


def start_connection() -> bytes:
    """Create start connection message"""
...
def finish_connection() -> bytes:
    """Create finish connection message"""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.WithEvent)
...
def start_session(payload: bytes, session_id: str) -> bytes:
    """Create start session message"""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.WithEvent)
...
def finish_session(session_id: str) -> bytes:
    """Create finish session message"""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.WithEvent)
...
def audio_only_client(payload: bytes, flag: MsgTypeFlagBits) -> bytes:
    """Create audio-only client message"""
