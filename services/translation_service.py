...
class TranslationResult:
    """翻译结果"""
    original_segments: List[TimedSegment]
    translated_segments: List[TimedSegment]
...
class TranslationService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise TranslationServiceError("未提供 OpenAI API 密钥")
        ...
        self.temperature = 0.3  # 较低的随机性，提高翻译一致性
    ...
    def translate_segments(self, segments: List[TimedSegment], 
        ...
        """
        if not segments:
            raise TranslationServiceError("输入片段列表为空")
        
        if target_language not in self.language_map:
            raise TranslationServiceError(f"不支持的目标语言: {target_language}")
        ...
        try:
            ...
            if source_language not in self.language_map:
                raise TranslationServiceError(f"不支持的源语言: {source_language}")
    ...
    def validate_translation_quality(self, original_segments: List[TimedSegment],
        ...
        # 长度一致性
        length_consistency = self._calculate_length_consistency(
            original_segments, translated_segments
        )
        
        # 综合评分
        overall_score = (timing_accuracy + length_consistency) / 2
...
4. 每行对应一个翻译结果"""
    ...
    def _call_translation_api_with_retry(self, prompt: str) -> str:
        ...
        raise TranslationServiceError(f"API调用失败，已重试{self.max_retries}次: {str(last_error)}")
    ...
    def _calculate_timing_accuracy(self, original_segments: List[TimedSegment],
        ...
        accurate_count = 0
        for orig, trans in zip(original_segments, translated_segments):
            if (abs(orig.start_time - trans.start_time) < 0.1 and 
                abs(orig.end_time - trans.end_time) < 0.1):
                accurate_count += 1
        
        return accurate_count / len(original_segments)
    
    def _calculate_length_consistency(self, original_segments: List[TimedSegment],
                                    translated_segments: List[TimedSegment]) -> float:
        """计算长度一致性"""
        if len(original_segments) != len(translated_segments):
            return 0.0
        ...
        for orig, trans in zip(original_segments, translated_segments):
            ...
            if orig_len == 0:
                if trans_len == 0:
                    ratios.append(1.0)  # 都为空，完全一致
                continue
            
            ratio = trans_len / orig_len
            # 合理的翻译长度比例
            if 0.5 <= ratio <= 2.0:
                consistency = 1.0 - abs(ratio - 1.0) * 0.5
            else:
                consistency = 0.0
            
            ratios.append(consistency)
