                ...
                while True:
                    try:
                        ...
                        if isinstance(response, str):
                            ...
                            logger.info(f"📥 收到消息 #{message_count}: {json.dumps(data, ensure_ascii=False)[:200]}")
                            ...
                            if data.get("message") == "success":
                                ...
                                logger.error(f"❌ ASR返回错误: {data}")
                                raise ProviderError(f"ASR错误: {data}")
                        else:
                            logger.info(f"📥 收到二进制响应: {len(response)} 字节")
                                
                    except asyncio.TimeoutError:
                        logger.info(f"⏱️ 接收超时，已收到 {message_count} 条消息")
                        break
